...
class Spodcast:
    ...
    def __init__(self, args):
        ...
        if args.prepare_feed is True:
            ...
            if os.path.exists(root_path):
                ...
                if (
                    not os.path.isfile(index_file_name)
                    or int(get_index_version(index_file_name))
                    < Spodcast.CONFIG.get_version_int()
                ):
                    rss_file = open(index_file_name, "w")
                    rss_file.write(
                        RSS_INDEX_CODE(
                            Spodcast.CONFIG.get_bin_path(),
                            os.path.basename(Spodcast.CONFIG.get_config_path()),
                            Spodcast.CONFIG.get_version_str(),
                        )
                    )
                    rss_file.close()
            else:
    ...
    def account(cls, line):
        ...
        if os.path.isdir(cred_directory):
            ...
            cred_filename = (
                ...
                + "-"
                + hashlib.md5(
                    username.encode("utf-8"), usedforsecurity=False
                ).hexdigest()
                + ".json"
            )
            ...
            conf = (
                ...
                .set_stored_credential_file(cred_location)
                .build()
            )
    ...
    def login(cls):
        cred_directory = Config.get_config_dir()
        credfiles = glob.glob(
            os.path.join(cred_directory, CREDENTIALS_PREFIX) + "-*.json"
        )
        if credfiles:
        ...
        while True:
            ...
            try:
                ...
                cls.SESSION = (
                    Session.Builder(conf).user_pass(user_name, password).create()
                )
                return
            except RuntimeError:
                pass
    ...
    def invoke_url(cls, url, tryCount=0):
        ...
        if "error" in responsejson:
            if tryCount < (cls.CONFIG.get_retry() - 1):
                Spodcast.LOG.warning(
                    f"Spotify API Error (try {tryCount + 1}) ({responsejson['error']['status']}): {responsejson['error']['message']}"
                )
                time.sleep(5)
                return cls.invoke_url(url, tryCount + 1)

            Spodcast.LOG.error(
                f"Spotify API Error ({responsejson['error']['status']}): {responsejson['error']['message']}"
            )
