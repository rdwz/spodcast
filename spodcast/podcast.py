...
from spodcast.feedgenerator import (
    RSS_FEED_CODE,
    RSS_FEED_FILE_NAME,
    RSS_FEED_SHOW_INDEX,
...
def get_episodes(show_id):
    ...
    episodes.sort(
        key=lambda x: datetime.strptime(
            f"{x.publish_time.year}-{x.publish_time.month}-{x.publish_time.day}T{x.publish_time.hour}:{x.publish_time.minute}:00Z",
            "%Y-%m-%dT%H:%M:%SZ",
        ),
        reverse=True,
    )
...
def download_stream(stream, filepath):
    ...
    if (
        ...
        (
            os.path.isfile(filepath)
            or (Spodcast.CONFIG.get_transcode() and os.path.isfile(mp3_filepath))
        )
        and Spodcast.CONFIG.get_skip_existing_files()
    ):
        return filepath, FILE_EXISTS, mimetype
    ...
    with open(filepath, "wb") as file:
        data = b""
        while data := stream.input_stream.stream().read(
            Spodcast.CONFIG.get_chunk_size()
        ):
            if data == b"":
                break
            file.write(data)
    ...
    try:
        ...
        if podcast_name is None:
            ...
            filename = clean_filename(
                release_date_clean + " - " + podcast_name + " - " + episode_name
            )
            show_directory = os.path.realpath(
                os.path.join(
                    Spodcast.CONFIG.get_root_path(), clean_filename(podcast_name) + "/"
                )
            )
            ...
            if download_url is None:
                ...
                stream = Spodcast.get_content_stream(
                    episode_stream_id, Spodcast.DOWNLOAD_QUALITY
                )
            ...
            if size == FILE_EXISTS:
                ...
                if Spodcast.CONFIG.get_rss_feed():
                    episode_info = {
                        ...
                        "date": time.strftime(
                            "%a, %d %b %Y %H:%M:%S +0000",
                            time.strptime(release_date, "%Y-%m-%dT%H:%M:%SZ"),
                        ),
                        "title": escape(episode_name),
                        "guid": uri,
                    ...
                    info_file = open(
                        os.path.join(
                            show_directory, f"{basename}.{RSS_FEED_INFO_EXTENSION}"
                        ),
                        "w",
                    )
            ...
            if Spodcast.CONFIG.get_rss_feed():
                show_index_file_name = os.path.join(
                    show_directory, f"{RSS_FEED_SHOW_INDEX}.{RSS_FEED_INFO_EXTENSION}"
                )
                if (
                    not os.path.isfile(show_index_file_name)
                    or int(get_index_version(show_index_file_name))
                    < Spodcast.CONFIG.get_version_int()
                ):
                    podcast_link, podcast_description, podcast_image = get_show_info(
                        podcast_id
                    )
                    show_info = {}
                    if os.path.isfile(show_index_file_name):
                        with open(show_index_file_name, encoding="utf-8") as file:
                            ...
                            file.close()
                    show_info["version"] = str(
                        RSS_FEED_VERSION + Spodcast.CONFIG.get_version_str()
                    )
                ...
                if (
                    not os.path.isfile(rss_file_name)
                    or int(get_index_version(rss_file_name))
                    < Spodcast.CONFIG.get_version_int()
                ):
